...
import { get_tags, create_tag, get_categories, create_category, create_post, get_post, edit_post } from '../../api/api'
...
export const PostEdit = () => {
  ...
  useEffect(() => {
    const setInnerItems = async () => {
      const selected_post = await get_post(match.params.postId)
      setPost(selected_post)
      setTitle(selected_post.title)
      setDescription(selected_post.description)
      setBody(selected_post.body)
      setPosterVideo(selected_post.posterVideo)
      setPosterImage(selected_post.posterImage)
  ...
  const onPosterSubmit = event => {
    ...
    return new Promise((resolve, reject) => {
      ...
      rf.onloadend = event => {
        ...
        data.append("image", event.target.result.split(",").pop()); //To delete 'data:image/png;base64,' otherwise imgbb won't process it.
        ...
        fetch(ImgbbUrl, { method: "POST", body: data })
          ...
          .then(result => {
            ...
            setPosterImage(result.data.url)
  ...
  const onSubmit = event => {
    ...
    setLoading(true)
    setPost(post)
    edit_post(post, match.params.postId)
    history.push('/posts')
  ...
  return loading ? <div>Loading...</div> : <>
    ...
    <form onSubmit={onGallerySubmit} encType="multipart/form-data">    
      ...
      <img style={{ display: posterImage ? "block" : "none", maxWidth: '100%' }}  
      />
      <button type="submit">Apply</button>
    </form>
      ...
      <InnerItem 
        ...
        // onRemoveItem={(id) => {
          ...
          // setSelectedTags([...selectedTags], selectedTags.push(id)) 
